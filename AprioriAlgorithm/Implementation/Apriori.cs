namespace AprioriAlgorithm
{
    ...
    public class Apriori : IApriori
    {
        ...
        // custom written
        private bool CheckIsSubsetCustom(string itemNames, string transactionItems)
        {
            List<string> transcationItemCombinationList = transactionItems.Split(',').ToList();
            //foreach (string itemInCombination in transcationItemCombinationList)
            //{
            //    if (itemInCombination.Equals(itemName))
            //        return true;
            //}
            List<string> itemNameList = itemNames.Split(',').ToList();
            foreach (string itemName in itemNameList)
            {
                if (!transcationItemCombinationList.Any(i => i.Equals(itemName)))
                    return false;
            }
        ...
        private string GenerateCandidate(string firstItem, string secondItem)
        {
            ...
            //int length = firstItem.Split(',').ToList().Count;

            if (length == 1)
            {
                return firstItem + secondItem;
                // for comma separated cadidates

                //var x = new List<string> { firstItem, secondItem };
                //x.Sort();
                //return x.ToString();
            ...
            else
            {
                ...
                //List<string> firstItemList = firstItem.Split(',').ToList();
                //List<string> secondItemList = secondItem.Split(',').ToList();

                //List<string> firstSubItemList = firstItemList;
                //List<string> secondSubItemList = secondItemList;
                //firstSubItemList.RemoveAt(firstItemList.Count - 1);
                //secondSubItemList.RemoveAt(secondItemList.Count - 1);

                //if(firstSubItemList == secondSubItemList)
                //{
                //    firstItemList.Add(secondItemList[secondItemList.Count - 1]);
                //    firstItemList.Sort();
                //    return string.Join(",", firstItemList);
                //}
